...
row=["","timeFirstTPacket","timeLastTPacket","timeFirstRPacket","timeLastRPacket",
"delaySum","tBytes","tPackets","rBytes",
"rPackets","lostPackets","timesForwarded","packetsDropped","bytesDropped","transmitterThroughput","recieverThroughput"]
writer.writerow(row)
...
for j in range(9):
	...
	for i in range(pt0):
		if(node0[i][2]>0.0):
			delaySum+=(node0[i][2]-node0[i][0])
		elif(node0[i][2]!=-1.0):
			lostPackets+=1
	for i in range(pt1):
		if(node1[i][2]>0.0):
			delaySum+=(node1[i][2]-node1[i][0])
		elif(node1[i][2]!=-1.0):
			lostPackets+=1
	timesForwarded=0
	for i in range(pt0):
	...
	transmitterThroughput=(bt0+bt1)/(timeLastTPacket-timeFirstTPacket)
	recieverThroughput=(br0+br1)/(timeLastRPacket-timeFirstRPacket)
