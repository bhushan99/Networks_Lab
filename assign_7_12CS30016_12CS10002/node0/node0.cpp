...
int main() {
	...
	while(1) {
		...
		if(strncmp(bufdg,"QSUCC",5)==0) {
			...
			int n1id=atoi(nid.c_str());
			struct sockaddr_in to1;
			to1.sin_family=AF_INET;
			to1.sin_addr.s_addr=inet_addr(nip.c_str());
			to1.sin_port=htons(atoi(nport.c_str()));
			int last=pre.ID;
			pre.ID=n1id;
			pre.ip=nip;
			pre.port=nport;
			...
			for(;x<fileinfo.size();x++){
				if(inRange(last,n1id,fileinfo[x].first)) {
					...
					if(sendto(socket_dgramid,scb.c_str(),MAX,0,(struct sockaddr *)&to1,sizeof(to1))==-1)
						perror("sendto");
				}
			}
			rem(last,n1id);
			...
			for(int i=7;i<strlen(bufdg);i++) {
				if(bufdg[i]==' ') {turn++;continue;}
				switch(turn){
					case 1: fip+=bufdg[i];break;
					case 2: fport+=bufdg[i];break;
				}
			}
			struct sockaddr_in to1;
			to1.sin_family=AF_INET;
			to1.sin_addr.s_addr=inet_addr(fip.c_str());
			to1.sin_port=htons(atoi(fport.c_str()));

			string tose="DELETE";
			if(sendto(socket_dgramid,tose.c_str(),MAX,0,(struct sockaddr *)&to1,sizeof(to1))==-1)
				perror("sendto");	
		}
